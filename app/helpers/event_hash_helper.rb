module EventHashHelper
  ...
  def setNewPaths
  @event_path = current_user.admin? ? new_admin_calendar_event_path : new_dashboard_calendar_event_path
 
  @home_path = current_user.admin? ? admin_calendar_path : dashboard_calendar_path
  end

  def setEditPaths
     @event_path = current_user.admin? ? edit_admin_calendar_event_path : edit_dashboard_calendar_event_path
  end
  ...
  def validate 
    ...
    resetSession #passes validations so reset session
    return true
  end 
